...
export async function POST(request) {
  ...
  const gps = {
    ...
    gcj02: gcoord.transform([Number(lng) || 0, Number(lat) || 0], gcoord.WGS84, gcoord.GCJ02),
    bd09: gcoord.transform([Number(lng) || 0, Number(lat) || 0], gcoord.WGS84, gcoord.BD09),
  };
  ...
  const insertNewRow = async () => {
    const response = await context.env.DB.prepare(
      'INSERT INTO records(gps, locate_datetime, create_datetime, update_datetime) VALUES(?1, ?2, ?3, ?3)',
    )
      .bind(JSON.stringify(gps), LocateDateTime, currentDateTime)
      .all();
  };

  const updateRowById = async (id) => {
    const response = await context.env.DB.prepare(
      'UPDATE records SET locate_datetime = ?2, update_datetime = ?3, gps = ?4  WHERE id = ?1',
    )
      .bind(id, LocateDateTime, currentDateTime, JSON.stringify(gps))
      .all();
  };
  ...
  const { results: latestItem } = await context.env.DB.prepare(
    "SELECT * FROM records WHERE locate_datetime >= date('now', 'start of day') ORDER BY update_datetime DESC LIMIT 0,1",
  )
    .bind()
    .all();
